...
function Track({ className, ...props }: ComponentProps<typeof BaseMeter.Track>) {
	return <BaseMeter.Track className={cx("bg-muted relative h-2 w-full overflow-hidden rounded-full", className)} {...props} />;
}

function Indicator({ className, ...props }: ComponentProps<typeof BaseMeter.Indicator>) {
	return <BaseMeter.Indicator className={cx("bg-primary h-full w-full flex-1 transition-all", className)} {...props} />;
}
